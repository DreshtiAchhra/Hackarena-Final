...
export const ReportPage: React.FC = () => {
  ...
  const getSeverityBadge = (severity: string) => {
    switch (severity) {
      ...
      default:
        return <span className="inline-flex items-center text-[10px] font-semibold uppercase px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-slate-400">Info</span>;
    }
  };
    ...
    <DashboardLayout>
      <div className="space-y-8 text-left relative">
        ...
        {exportNotification.show && (
          <div className="fixed bottom-6 right-6 z-50 p-4 bg-slate-900 border border-accent rounded-xl shadow-accent-glow flex items-center space-x-3 text-sm text-slate-200 animate-bounce">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-slate-400 text-sm">Download official report logs or inspect category violations lists.</p>
          </div>
          ...
          <div className="flex items-center space-x-3 self-start">
            <button
              ...
              className="flex items-center space-x-1.5 px-4 py-2.5 bg-slate-900 hover:bg-slate-850 text-slate-300 text-xs font-bold rounded-lg border border-slate-800 transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center space-x-1.5 px-4 py-2.5 bg-accent hover:bg-indigo-750 text-white text-xs font-bold rounded-lg transition-all duration-200 hover:-translate-y-0.5 shadow-md shadow-accent-glow"
            >
        ...
        <div className="p-6 bg-slate-900/60 border border-slate-800 rounded-2xl grid grid-cols-1 md:grid-cols-4 gap-6 items-center">
          <div className="space-y-1">
            <span className="text-xs text-slate-500 font-bold uppercase tracking-wider">Executive Rating</span>
            <h3 className="text-2xl font-black text-white">shoppycart.io</h3>
            <span className="text-xs text-slate-400">Index updated: June 27, 2026</span>
          </div>
          
          <div className="flex items-center space-x-3.5 p-3.5 bg-slate-950/40 rounded-xl border border-slate-800/80">
          ...
          <div className="flex items-center space-x-3.5 p-3.5 bg-slate-950/40 rounded-xl border border-slate-800/80">
            <div className="text-center">
              ...
              <span className="text-xl font-black text-indigo-400 mt-0.5 block">{mockMetadata.totalPages}</span>
            </div>
            <div className="flex-1 text-right">
              <span className="text-[10px] text-slate-500 font-mono block">Coverage</span>
              <span className="text-xs font-bold text-slate-350 block mt-0.5">{mockMetadata.coverage}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card border border-slate-800 rounded-2xl overflow-hidden shadow-lg">
          ...
          {/* Table Toolbar */}
          <div className="p-5 bg-slate-900 border-b border-slate-850 flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4">
            
            {/* Search Input */}
            <div className="flex items-center space-x-2.5 px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg focus-within:border-accent transition-colors flex-1 max-w-md">
              ...
              <input
                ...
                className="bg-transparent border-0 outline-none text-slate-200 text-xs placeholder-slate-500 w-full"
              />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select
                ...
                className="bg-slate-950 border border-slate-800 text-slate-300 text-xs py-2 px-3.5 rounded-lg focus:border-accent outline-none"
              >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-900/60 border-b border-slate-850 text-[10px] uppercase tracking-wider text-slate-500 font-extrabold select-none">
              ...
              <tbody className="divide-y divide-slate-850 text-xs">
                {filteredIssues.length > 0 ? (
                  filteredIssues.map((issue, idx) => (
                    <tr key={issue.id} className="hover:bg-slate-900/30 transition-colors">
                      <td className="py-4.5 px-6 font-mono text-slate-500 font-semibold">{issue.id}</td>
                      <td className="py-4.5 px-6">
                        <span className="font-bold text-slate-200 block mb-0.5">{issue.title}</span>
                        <span className="text-slate-450 line-clamp-1">{issue.description}</span>
                      </td>
                      <td className="py-4.5 px-6">{getSeverityBadge(issue.severity)}</td>
                      <td className="py-4.5 px-6">
                        <span className="text-[10px] uppercase font-bold text-slate-400 bg-slate-900 px-2 py-0.5 rounded border border-slate-850">
                          {issue.category}
                        </span>
                      </td>
                      <td className="py-4.5 px-6 text-right font-mono text-slate-400">{issue.pageUrl}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
